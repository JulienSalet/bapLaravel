                ...
                <div class="card-block">
                    <table class="table table-hover demo-table-search table-responsive-block" id="tableWithSearch">
                        ...
                        <tr>
                            ...
                            <th>Personne qui a reservée</th>
                            <th>Date reservée</th>
                            <th>Plage horaire reservée</th>
                        ...
                        @foreach($reservations AS $key => $reservation)
                            <tr>
                                ...
                                <td class="v-align-middle semi-bold">
                                    {{ $reservation->getSalle ? $reservation->getSalle->titre : '—' }}
                                </td>
                                <td class="v-align-middle semi-bold">
                                    {!! $reservation->getUser ? $reservation->getUser->first_name . ' <strong>' .$reservation->getUser->last_name .'</strong>': '—'  !!}
                                </td>
                                <td class="v-align-middle semi-bold">
                                ...
                                <td style="display: flex;" class="v-align-middle">
                                    <a href="{{ action('Admin\Cms\SalleController@edit', $reservation->id) }}"
                                       class="btn btn-primary mr-3">
                                        Réenvoyer un mail
                                    </a>
                                    <form action="{{ action('Admin\Cms\SalleController@destroy', $reservation->id) }}"
                                          method="POST">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Horaire de la réservation
                            <strong>#{{ $reservation->id}}</strong></h5>
...
{{--                                {{ dd(json_decode($reservation->horaire)) }}--}}
                                @foreach(json_decode($reservation->horaire) as $startInfo => $value)
                                    <span>{{ $startInfo === "start_time" ? "De"  :  "A" }} {!! '<strong>' . $value . '</strong>' !!}</span>
                                @endforeach
                            </div>
                        @endif
